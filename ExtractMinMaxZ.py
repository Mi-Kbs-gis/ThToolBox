...
class ExtractMinMaxZ(GeoAlgorithm):
    ...
    def defineCharacteristics(self):
        ...
        self.name, self.i18n_name = self.trAlgorithm('Extract Min/Max Z-Values')
        ...
        self.addOutput(OutputVector(self.OUTPUT, self.tr('with Min/Max Z-Values')))
    ...
    def processAlgorithm(self, progress):
        ...
        writer = self.getOutputFromName(self.OUTPUT).getVectorWriter(
            fields,
            QgsWKBTypes.multiType(QGis.fromOldWkbType(vectorLayer.wkbType())),
            vectorLayer.crs())
        ...
        for feature in iter:
            ...
            zmin, zmax=self.computeGeometrieValuesZ(geom)
            attrs.append(zmin)
            attrs.append(zmax)
            try:
                out_feat = QgsFeature()
